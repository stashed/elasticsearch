...
Install Elasticsearch {{ $v.Major }}.{{ $v.Minor }}.{{ $v.Patch }} backup or restore plugin for Stash as below.

```console
helm repo add appscode https://charts.appscode.com/stable/
helm repo update
helm install stash-elasticsearch-{{ .version }} appscode/stash-elasticsearch --version={{ .version }} --namespace=kube-system
```
...
Uninstall Elasticsearch {{ $v.Major }}.{{ $v.Minor }}.{{ $v.Patch }} backup or restore plugin for Stash as below.

```console
helm uninstall stash-elasticsearch-{{ .version }} --namespace=kube-system
```
