...
$ helm repo update
$ helm install stash-elasticsearch-5.6.4-rc.20200827 appscode/stash-elasticsearch -n kube-system --version=5.6.4-rc.20200827
```
...
To install the chart with the release name `stash-elasticsearch-5.6.4-rc.20200827`:
...
To uninstall/delete the `stash-elasticsearch-5.6.4-rc.20200827`:

```console
$ helm delete stash-elasticsearch-5.6.4-rc.20200827 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore Elasticsearch database. This is usually same as the database version it can backup. | `5.6.4-rc.20200827`   |
...
```console
$ helm install stash-elasticsearch-5.6.4-rc.20200827 appscode/stash-elasticsearch -n kube-system --version=5.6.4-rc.20200827 --set image.registry=stashed
```
...
```console
$ helm install stash-elasticsearch-5.6.4-rc.20200827 appscode/stash-elasticsearch -n kube-system --version=5.6.4-rc.20200827 --values values.yaml
```
