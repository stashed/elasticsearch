...
$ helm repo update
$ helm install stash-elasticsearch-6.3.0-v4 appscode/stash-elasticsearch -n kube-system --version=6.3.0-v4
```
...
To install the chart with the release name `stash-elasticsearch-6.3.0-v4`:
...
To uninstall/delete the `stash-elasticsearch-6.3.0-v4`:

```console
$ helm delete stash-elasticsearch-6.3.0-v4 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore Elasticsearch database. This is usually same as the database version it can backup. | `6.3.0-v4`            |
...
```console
$ helm install stash-elasticsearch-6.3.0-v4 appscode/stash-elasticsearch -n kube-system --version=6.3.0-v4 --set image.registry=stashed
```
...
```console
$ helm install stash-elasticsearch-6.3.0-v4 appscode/stash-elasticsearch -n kube-system --version=6.3.0-v4 --values values.yaml
```
