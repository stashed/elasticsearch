...
$ helm repo update
$ helm install stash-elasticsearch-v2020.11.06 appscode/stash-elasticsearch -n kube-system --version=v2020.11.06
```
...
To install the chart with the release name `stash-elasticsearch-v2020.11.06`:
...
To uninstall/delete the `stash-elasticsearch-v2020.11.06`:

```console
$ helm delete stash-elasticsearch-v2020.11.06 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore Elasticsearch database. This is usually same as the database version it can backup. | `v2020.11.06`         |
...
```console
$ helm install stash-elasticsearch-v2020.11.06 appscode/stash-elasticsearch -n kube-system --version=v2020.11.06 --set image.registry=stashed
```
...
```console
$ helm install stash-elasticsearch-v2020.11.06 appscode/stash-elasticsearch -n kube-system --version=v2020.11.06 --values values.yaml
```
