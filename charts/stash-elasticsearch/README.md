...
$ helm repo update
$ helm install stash-elasticsearch-7.2.0-beta.20200826 appscode/stash-elasticsearch -n kube-system --version=7.2.0-beta.20200826
```
...
To install the chart with the release name `stash-elasticsearch-7.2.0-beta.20200826`:
...
To uninstall/delete the `stash-elasticsearch-7.2.0-beta.20200826`:

```console
$ helm delete stash-elasticsearch-7.2.0-beta.20200826 -n kube-system
```
...
| image.tag        | Tag of the image that is used to backup/restore Elasticsearch database. This is usually same as the database version it can backup. | `7.2.0-beta.20200826` |
...
```console
$ helm install stash-elasticsearch-7.2.0-beta.20200826 appscode/stash-elasticsearch -n kube-system --version=7.2.0-beta.20200826 --set image.registry=stashed
```
...
```console
$ helm install stash-elasticsearch-7.2.0-beta.20200826 appscode/stash-elasticsearch -n kube-system --version=7.2.0-beta.20200826 --values values.yaml
```
